...
const patchSchema = z.object({
  id: z.string().min(1),
  status: z.enum(["planned", "done", "skipped", "unavailable"]),
  ...
  // Konteks opsional, khususnya untuk "unavailable" — kenapa tidak bisa?
...
export async function PATCH(req: NextRequest) {
  return handle(async () => {
    ...
    const ok = await setOccurrenceStatus(ctx, body.id, body.status, {
